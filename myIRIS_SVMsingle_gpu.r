...
## TO ACTIVATE CUDA USE: install.packages('liquidSVM',configure.args="native /usr/local/cuda")
...
data = read.table('/home/microcarbon/Desktop/iris_tab_big.txt', header = TRUE)
...
tic("time elapsed")
mytest <- svm(Sp~SL+SW+PL+PW, mysample, threads=1, display=1, gpus=1, max_gamma=125, do.select=TRUE, predict.prob=FALSE)
